...
async def start_pm(client, message: Message, _):
    ...
    if len(message.text.split()) > 1:
        ...
        KRITI = await message.reply_text(f"**ʜᴇʏ ʙᴧʙʏ {message.from_user.mention}**")
        await asyncio.sleep(0.4)
        await KRITI.edit_text("**ɪ ᴧᴍ ʏᴏᴜʀ ᴏᴡɴ ᴍᴜsɪᴄ ʙᴏᴛ..🦋**")
        await asyncio.sleep(0.4)
        await KRITI.edit_text("**ʜᴏᴡ ᴧʀᴇ ʏᴏᴜ ᴛᴏᴅᴧʏ.....??**")
        await asyncio.sleep(0.4)
        await KRITI.delete()
